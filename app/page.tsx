...
const EMPTY_DATA: MarketingAgencyCRMData = {
  ...
  banner: {
    status: "healthy",
    constraint: "Connect your CRM to view live pipeline insights.",
    recommendation: "Link a lead source to keep deals, tasks, and activity data up to date.",
...
function mapBoardToCrmData(board: BoardPayload | null, liveError: string): MarketingAgencyCRMData {
  if (!board) {
    ...
    return {
      ...
      banner: {
        status: "warning",
        constraint: "Live sync is currently unavailable.",
  ...
  return {
    ...
    banner: {
      ...
      constraint: toText(board.banner?.constraint, liveError ? "Live sync is currently unavailable." : EMPTY_DATA.banner.constraint),
