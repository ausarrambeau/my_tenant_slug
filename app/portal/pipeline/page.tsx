...
const EMPTY_DATA: MarketingAgencyCRMData = {
  ...
  banner: {
    status: "healthy",
    constraint: "Pipeline updates will appear here.",
    recommendation: "Connect your lead source to track deals in real time.",
...
function mapBoardToCrmData(board: BoardPayload | null, liveError: string): MarketingAgencyCRMData {
  if (!board) {
    ...
    return {
      ...
      banner: {
        status: "warning",
        constraint: "We couldn't load live CRM data.",
  ...
  return {
    ...
    banner: {
      ...
      constraint: toText(board.banner?.constraint, liveError ? "We couldn't load live CRM data." : EMPTY_DATA.banner.constraint),
