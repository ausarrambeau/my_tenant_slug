...
const EMPTY_DATA: MarketingAgencyCRMData = {
  ...
  banner: {
    status: "healthy",
    constraint: "No live pipeline data yet.",
    recommendation: "Connect a lead source to start tracking deals here.",
...
function mapBoardToCrmData(board: BoardPayload | null, liveError: string): MarketingAgencyCRMData {
  if (!board) {
    ...
    return {
      ...
      banner: {
        status: "warning",
        constraint: "Live sync is temporarily unavailable.",
  ...
  return {
    ...
    banner: {
      ...
      constraint: toText(board.banner?.constraint, liveError ? "Live sync is temporarily unavailable." : EMPTY_DATA.banner.constraint),
